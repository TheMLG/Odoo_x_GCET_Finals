...
export const useCartStore = create<CartState>()(
  persist(
      ...
      fetchCart: async () => {
        try {
          ...
          if (!cartData || !cartData.items) {
            ...
            // localStorage.removeItem('rental-cart'); // No longer needed manual removal if handled by store or logic
            return;
          }
    ...
    }),
    {
      ...
      partialize: (state) => ({ appliedCoupon: state.appliedCoupon }), // Only persist appliedCoupon
    }
  )
);
